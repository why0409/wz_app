...
@Api(tags = "测评活动接口")
@RestController
@RequestMapping("/assessment/activity")
public class AssessmentActivityController extends BaseController {
    @Autowired
...
//    @PreAuthorize("@ss.hasPermi('assessment:activity:add')")
    ...
    public AjaxResult add(@RequestBody AssessmentActivity assessmentActivity) {
        ...
        assessmentActivity.setStatus("0"); // 默认未开始
        return toAjax(assessmentActivityService.save(assessmentActivity));
    }
