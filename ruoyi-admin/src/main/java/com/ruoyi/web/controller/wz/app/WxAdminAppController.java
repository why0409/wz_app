...
@Api(tags = "微信端测评活动管理员接口")
@RestController
@RequestMapping("/applet/wx/adminApp")
public class WxAdminAppController extends BaseController {
    @Autowired
...
//    @PreAuthorize("@ss.hasPermi('wx:admin:remove')")
    @GetMapping("/{phone}")
    @ApiOperation("判断进入测评用户身份")
    public AjaxResult isAdmin(@PathVariable String phone) {
        long rows = wxAdminService.countByPhone(phone);
        return AjaxResult.success(rows > 0);
    }
}