...
import com.ruoyi.web.annotation.LoginInfo;
...
public class UserInfoController extends BaseController {
     ...
     * 根据手机号查询权限
     */
    @LoginInfo(description = "微信用户登录获取权限")
    ...
    public AjaxResult updatePerCen(@RequestBody WxUserLogininfo wxUserLogininfo) {
        ...
        try {
            resp = HttpUtil.post(ApiUrl, JSONObject.toJSONString(wxUserLogininfo));
            logger.info("======================"+resp+"======================");
            logger.info("======================  用户信息同步成功-极视角  ======================");

            List<WxUserLogininfo> list = wxUserLogininfoService.selectByPhone(wxUserLogininfo.getPhone());
            if (list.size() > 0) {
                //更新本地库
                wxUserLogininfoService.updateWxUserLogininfo(wxUserLogininfo);
            }else {
